...
function TeacherReports() {
  ...
  useEffect(() => {
    const fetchTrends = async () => {
      try {
        const response = await axios.get("http://localhost:4242/teacher/api/teacher/reports/trends", {
          params: { ...filters, teacherId: 1 }, // Replace with actual teacher ID from authentication
        });
  ...
  useEffect(() => {
    const fetchClassPerformance = async () => {
      try {
        const response = await axios.get("http://localhost:4242/teacher/api/teacher/reports/class-performance", {
  ...
  const handleExport = async () => {
    try {
      const response = await axios.get("http://localhost:4242/teacher/api/teacher/reports/export", {
        params: { teacherId: 1, ...filters }, // Replace with actual teacher ID from authentication
        responseType: "blob",
      });
