...
student.get("/api/student/dashboard", async (req, res) => {
  ...
  try {
    ...
    const totalDaysResult = await connection.query(
    ...
    // Fetch days present
    const daysPresentResult = await connection.query(
    ...
    // Fetch days absent
    const daysAbsentResult = await connection.query(
      "SELECT COUNT(*) FROM Attendance WHERE student_id = $1 AND status = 'absent'",
      [studentId]
    );
    const daysAbsent = parseInt(daysAbsentResult.rows[0].count);

    // Fetch days late
    const daysLateResult = await connection.query(
      "SELECT COUNT(*) FROM Attendance WHERE student_id = $1 AND status = 'late'",
      [studentId]
    );
