...
admin.post("/api/admin/classes", async (req, res) => {
    ...
    try {
        const result = await connection.query("INSERT INTO Classes (name, teacher_id) VALUES ($1, $2) RETURNING *", [name, teacherId]);
...
admin.put("/api/admin/classes/:id", async (req, res) => {
    ...
    const { name, teacherId } = req.body;
    try {
        const result = await connection.query("UPDATE Classes SET name = $1, teacher_id = $2 WHERE id = $3 RETURNING *", [name, teacherId, id]);
...
admin.post("/api/admin/classes/:id/students", async (req, res) => {
    ...
    try {
        const result = await connection.query("INSERT INTO Students (user_id, class_id) VALUES ($1, $2) RETURNING *", [userId, id]);
...
admin.delete("/api/admin/classes/:id/students/:userId", async (req, res) => {
    ...
    try {
        const result = await connection.query("DELETE FROM Students WHERE class_id = $1 AND user_id = $2 RETURNING *", [id, userId]);
        if (result.rows.length === 0) {
...
admin.put("/api/admin/attendance/:id", async (req, res) => {
    ...
    try {
        const result = await connection.query("UPDATE Attendance SET status = $1 WHERE id = $2 RETURNING *", [status, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: "Attendance record not found." });
        }
...
admin.post("/api/admin/attendance", async (req, res) => {
    ...
    try {
        const result = await connection.query("INSERT INTO Attendance (student_id, class_id, date, status) VALUES ($1, $2, $3, $4) RETURNING *", [studentId, classId, date, status]);
