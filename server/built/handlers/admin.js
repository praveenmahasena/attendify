...
export async function adminLogin(req, res) {
    try {
        ...
        res.cookie("cookie", jwt_cookie, {
            ...
            maxAge: 3 * 24 * 60 * 60 * 1000, // Cookie expiration: 3 days in milliseconds
        });
...
export async function overView(req, res) {
    try {
        // Total Users
        const totalUsersResult = await connection.query("SELECT COUNT(*) FROM Users");
        const totalUsers = parseInt(totalUsersResult.rows[0].count);
        // Active Classes
        const activeClassesResult = await connection.query("SELECT COUNT(*) FROM Classes");
...
export async function attendance(req, res) {
    try {
      ...
      ORDER BY date ASC
    `);
        const attendanceTrends = result.rows.map((row) => ({
...
export async function performance(req, res) {
    try {
      ...
      GROUP BY c.name
    `);
        const classPerformance = result.rows.map((row) => ({
...
export async function adminAddUsers(req, res) {
    ...
    try {
        ...
        const result = await connection.query("INSERT INTO Users (name, email, password, role) VALUES ($1, $2, $3, $4) RETURNING *", [name, email, hashedPassword, role]);
...
export async function adminUserUpdate(req, res) {
    ...
    try {
        const result = await connection.query("UPDATE Users SET name = $1, email = $2, role = $3 WHERE id = $4 RETURNING *", [name, email, role, id]);
...
export async function addClasses(req, res) {
    ...
    try {
        const result = await pool.query("INSERT INTO Classes (name, teacher_id) VALUES ($1, $2) RETURNING *", [name, teacherId]);
...
export async function updateClasses(req, res) {
    ...
    const { name, teacherId } = req.body;
    try {
        const result = await pool.query("UPDATE Classes SET name = $1, teacher_id = $2 WHERE id = $3 RETURNING *", [name, teacherId, id]);
        if (result.rows.length === 0) {
...
export async function assignstudentclass(req, res) {
    ...
    try {
        const result = await pool.query("INSERT INTO Students (user_id, class_id) VALUES ($1, $2) RETURNING *", [userId, id]);
...
export async function deleteStudentFromClass(req, res) {
    ...
    try {
        const result = await pool.query("DELETE FROM Students WHERE class_id = $1 AND user_id = $2 RETURNING *", [id, userId]);
        if (result.rows.length === 0) {
